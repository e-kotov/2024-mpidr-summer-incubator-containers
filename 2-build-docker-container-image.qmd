...
To achieve this goal, find a few archived R packages on [CRAN](https://cran.r-project.org/){target="_blank"}.
A few notable examples are [`rgdal`](https://cran.r-project.org/package=rgdal){target="_blank"}, [`MortalitySmooth`](https://cran.r-project.org/package=MortalitySmooth){target="_blank"} and many others.
Remember, "40% of all packages ever in CRAN got at one point archived"\[\^1\].
CRAN does not have it's own section with archived packages, so you might want to look at [CRANhaven Dashboard](https://www.cranhaven.org/dashboard-live.html){target="_blank"} is also a great place to start, over there you can find recently archived packages.

### Choose the Rocker Image version

Depending on which archived package you selected, you must first find out which [Rocker image](https://rocker-project.org/){target="_blank"} version to use. For example, if you selected `MortalitySmooth` package archived on 2020-12-10. Use internet search to find the release dates of R versions released just before this date. Rocker images are configured to use CRAN snapshot on the date of the R version release. That is, if you would use Rocker RStudio container image with R v4.1.0 released on 18th May 2021, the R package installer in the container will think it is 18th May 2021 and will try to install the package from the CRAN snapshot from that date. If the package was archived after that date, you will not be able to install it using the standard `install.packages()` function. You will have to use `remotes::install_version()` function from the `remotes` package.
So your options are:
